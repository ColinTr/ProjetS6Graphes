 ...
 *
 * \param GRAGraphe CGraphe (en lvalue) que l'on souhaite recopier
 *
 */
 ...
 * \brief Constructeur de recopie de CGraphe
 *
 * \param GRAGraphe CGraphe (en rvalue) que l'on souhaite recopier
 *
 */
CGraphe::CGraphe(CGraphe && GRAGraphe) {
    unsigned int uiIterateur;
 ...
 *
 * Cette méthode appelle les fonctions du CParser pour créer un CGraphe à partir du fichier donné en paramètre
 *
 */
CGraphe::CGraphe(char * pcAdresseFichier)
{
	...
	if (fichier) { // Si on a pu ouvrir
		...
		//Si le nombre de valeurs trouvées est incohérent, exception.
		...
		//Puis on parcourt notre tableau de numéros de sommets pour remplir notre nouveau graphe en prenant soin de correctement définir le numéro maximum
 ...
 *
 * Cette méthode désalloue et supprime les CSommets qui étaient présents dans les tableaux du CGraphe dans lequel on veut recopier le CGraphe passé en paramètre. De plus, les CSommets sont copiés entièrement et non juste leurs adresses.
 *
 */
 ...
 * \brief Méthode qui permet d'ajouter un Sommet à notre graphe, l'id de celui-ci est donné en parametre.
 *
 ...
 *
 * \param uiIdSommet unsigned int qui spécifie l'id du CSommet que l'on souhaite modifier
 * \param uiNumero unsigned int qui spécifie l'identifiant que l'on souhaite attribuer à notre CSommet.
 *
